...
module.exports.handleEvent = function({ api, event, client, __GLOBAL, Users }) {
	...
	if (event.body.indexOf("bakit")==0 || (event.body.indexOf("ansabe")==0 || (event.body.indexOf("anyare?","anong nangyare?")==0 || (event.body.indexOf("bakit?","Bakit","bakit kaya?", "Bakit?")==0)))) {
			api.sendMessage("di ko alam nigga", event.threadID, (e, info) => {
      setTimeout(() => {
