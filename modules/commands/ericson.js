module.exports.config = {
  ...
  version: "1.0.0",
  hasPermission: 0,
  description: "ericson bot that can download facebook videos, sing, search image, search lyrics and more..",
  usePrefix: false,
  credits: "Deku and Modified by  ericson終.(botpack)",
  usages: "[help]",
  commandCategory: "ericson",
  cooldowns: 10,
};
...
Just type <ericson can you sing (title of song)> or <midoriya sing (title of song)> as long as there is the word sing and the title of the song, it can automatically detect it


•ERICSON CAN DOWNLOAD FACEBOOK VIDEOS.

Just type <ericson can you download this facebook video (fb video url)> or <ericson download this facebook video (fb video url)> as long as there is the word ‘facebook video’ or ‘download’ and the fb video url, it can automatically detect it.


•ERICSON CAN SEARCH IMAGES.

Just type <ericson can you search an image of (your search)> or <ericson image of (your search)> as long as there is the word ‘image’ or ‘image of’ and your search, it can automatically detect it.


•ERICSON CAN SEARCH VIDEO FROM YOUTUBE.

Just type <ericson can you send a video of (your search)> or <harold video of (your search)> and more... as long as there's the word ‘video of’ and you search, it will automatically send a video

•ERICSON CAN TALK.

Just type <ericson say (message)>.

•ERICSON CAN SEND A LYRICS.

Just type <ericson can you give me the lyrics of (song name)> or <ericson give me a lyrics of (song name)> as long as there's a word ‘lyrics of’ and the song name it will be automatically detected.

•ERICSON CAN DOWNLOAD TIKTOK VIDEOS USING LINK

Just type <ericson can you download this tiktok link (and tiktok your link)> or <midoriya download this tiktok link (your tiktok link)>

•ERICSON CAN SEND A TIKTOK VIDEO

Just type <ericson send the tiktok video of (your search)> or <harold send the tiktok video of (your search)> or <midoriya send a tiktok video of (your search)>
...
if (!txt) return api.sendMessage(rand1, threadID, messageID);
 if (txt.toLowerCase().startsWith("thank you") || txt.toLowerCase().startsWith("thank")) return api.sendMessage("You're welcome user 🤗", threadID, messageID);
  ...
  if (txt.toLowerCase().includes("who created you") || txt.toLowerCase().includes("who's your master") || txt.toLowerCase().includes("who create you") || txt.toLowerCase().includes("who's your creator")) return api.sendMessage("I'm ericson Modified by ericson終. and original this code by Deku.", event.threadID, event.messageID);
